...
> 当前版本 **v0.0.1**（开发中）。界面已支持：默认平铺全部 todo（点日历/任务卡片筛选，再按一次取消；点右上角 home 图标回平铺）、添加 todo（输入框回车，支持行内 `@日期` `+任务`）、状态切换（`[ ]`→`[/]`→`[x]`，自动记录开始/完成日期）、任务卡片与任务详情、拖拽迁移（拖到日历某天即整行移动）、拖拽排序；界面内创建任务 / 向任务加 todo 仍在开发中（可直接编辑文件）。
...
> 在界面里直接编辑 `inbox/<今天>.md` 仍然可行（外部改动刷新后生效）；创建新任务、向已有任务加 todo 目前也只能编辑文件（界面入口排在 M2）。
