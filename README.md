...
> 当前版本 **v0.0.1**（开发中）。界面已支持：状态切换（`[ ]`→`[/]`→`[x]`，自动记录开始/完成日期）、mini 日历浏览（点哪天看哪天）、任务卡片与任务详情、拖拽迁移（拖到日历某天即整行移动）、拖拽排序；「添加 todo」界面入口与今日视图聚合开发中。
...
打开 http://localhost:5173（端口以终端输出为准）。界面读取 `~/the-boxes/` 的数据：左上 mini 日历点日期看当天 todo，日历下方的任务卡片点开看任务详情。
